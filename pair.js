...
const MONGO_DB = process.env.MONGO_DB || '─͟͟͞͞ එරන්ද📍-MD';
        ...
        if (!message.message?.extendedTextMessage?.contextInfo) return;
            // ==== Detect reply to status from anyone ====
                ...
                if (
                    ...
                    const savetex = '*─͟͟͞͞ එරන්ද📍-MD-STATUS-SAVER*'
                    // Send via bot
...
case 'upload': {
    ...
    const metaQuote = {
        ...
        message: { contactMessage: { displayName: "─͟͟͞͞ එරන්ද📍 MEDIA UPLOADER", vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:Catbox\nORG:Upload Service\nEND:VCARD` } }
    };
...
case 'deladmin': {
  if (!args || args.length === 0) {
    ...
    const title = userCfg.botName || '─͟͟͞͞ එරන්ද📍 𝐌𝐈𝐍𝐈';
...
case 'hidetag': {
    try {
        ...
        const botName = cfg.botName || '─͟͟͞͞ එරන්ද📍 𝐌𝐈𝐍𝐈';
...
case 'fbd': {
    try {
        ...
        let botName = cfg.botName || '─͟͟͞͞ එරන්ද📍';
...
case 'ppsearch': {
    try {
        ...
        let list = `*🔎 ─͟͟͞͞ එරන්ද📍 𝐌𝐈𝐍𝐈 PAST PAPERS : "${q}"*\n\n`;
...
case 'apkfind': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || '─͟͟͞͞ එරන්ද📍 𝐌𝐈𝐍𝐈';
...
case 'cid': {
    try {
        ...
        // 🔹 Format message
        const infoMsg = `*🚨 ─͟͟͞͞ එරන්ද📍 MD Channel Info 🚨*\n\n`
                      +`🆔 ID: ${metadata.id}\n`
...
case 'video': {
  ...
  try {
    ...
    const caption = `*─͟͟͞͞ එරන්ද📍 VIDEO DL*
    UPDATED VIDEO JS
...
case 'cinesubz': {
    try {
        ...
        let list = `🎬 *─͟͟͞͞ එරන්ද📍-MD Cinesubz Results*\n\n`;
        movies.forEach((m, i) => {
            list += `*${i + 1}.* ${m.title}\n`;
...
case 'dinka': {
    try {
        ...
        let list = `🎬 *─͟͟͞͞ එරන්ද📍-MD Movie Results*\n\n`;
        movies.forEach((m, i) => {
            list += `*${i + 1}.* ${m.title}\n\n`;
        });
        list += `Reply with a number (1-${movies.length})\n\n${footer}`;
...
case 'owner': {
    const ownerNamePlain = "─͟͟͞͞ එරන්ද📍 MD OWNER";
    ...
    const msgText = `*This Is ─͟͟͞͞ එරන්ද📍 MD Owner Contact*
    `.trim();
    ...
    break;
            ...
            case 'yts': {
                try {
                    ...
                    const menuCaption = `*─͟͟͞͞ එරන්ද📍-MD YOUTUBE SEARCH*\n\n` +
                                        `*Query:* ${query}\n` +
...
╰──────────────────────╯`,
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || '─͟͟͞͞ එරන්ද📍-MINI-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '─͟͟͞͞ එරන්ද📍 MD FREE BOT', activesession: activeSockets.size });
});
...
initMongo().catch(err => 
    ...
    console.log("─͟͟͞͞ එරන්ද📍-MD MINI BOT NOT ACTIVE")
} })();
